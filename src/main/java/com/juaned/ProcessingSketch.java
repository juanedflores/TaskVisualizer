...
public class ProcessingSketch extends PApplet {
  ...
  double keydataoffset =
      canvasHeight * 0.04301; // 20, y distance. font line height. might depend on font size.
  double keypercentoffset =
      canvasWidth * 0.1125; // 90, x distance between time and percentage text.
  ...
  public void setup() {
    ...
    angles =
        new float[3]
            [anglecount]; // [seconds][degrees][value determining whether on left or right side of
    // chart]
    ...
    for (int i = 0; i < values.length(); i++) {
      ...
      if (task.has("displaytag")) {
        ...
        startx[index] =
            PApplet.map(
                    task.getFloat("startx"), (float) 0, (float) 24.0, (float) 0.0, (float) 360.0)
                + 90; // +90
        // to
        index++;
      } else {
        miscangles[miscindex] = ceil(map(task.getInt("seconds"), 0, secondsinday, 0, 360));
        startxmisc[miscindex] =
            map(task.getFloat("startx"), (float) 0, (float) 24.0, (float) 0.0, (float) 360.0) + 90;
        miscindex++;
      }
    }
  ...
  void pieTime(float diameter) {
    ...
    stroke(0);
    float sunriseX =
        chartcenterX + cos(radians(map(sunrisehrfract, 0, 24, 0, 360) + 90)) * (diameter / 2);
    float sunriseY =
        chartcenterY + sin(radians(map(sunrisehrfract, 0, 24, 0, 360) + 90)) * (diameter / 2);
    line(chartcenterX, chartcenterY, sunriseX, sunriseY);
    float sunsetX =
        chartcenterX + cos(radians(map(sunsethrfract, 0, 24, 0, 360) + 90)) * (diameter / 2);
    float sunsetY =
        chartcenterY + sin(radians(map(sunsethrfract, 0, 24, 0, 360) + 90)) * (diameter / 2);
    ...
    arc(
        ...
        diameter + 25,
        diameter + 25,
        radians(map(sunrisehrfract, 0, 24, 0, 360) + 90),
  ...
  void pieChart(float diameter, float[][] data, int[] miscdata) {
    ...
    for (int i = 0; i < miscdata.length; i++) {
      ...
      arc(
          ...
          diameter,
          diameter,
          radians(startxmisc[i]),
          radians(startxmisc[i]) + radians(miscdata[i]),
          PIE);
    }
    ...
    for (int i = 0; i < data[0].length; i++) {
      ...
      arc(
          ...
          diameter,
          radians(startx[i]),
          radians(startx[i]) + radians(data[1][i]),
          PIE);
      ...
      if (drawkey) {
        if (right) {
          ...
          rectMode(CENTER);
          rect(
              (float) keyposXright - (float) keyrectXoffset,
              (float) keyposYright - (float) keyrectYoffset,
              (float) 16,
              (float) 16);
          ...
          line(
              ...
              (float) keyposYright - (float) keylineYoffset,
              (float) keyposXright - (float) keylineXoffset,
          ...
          text(
              strpercent,
              (float) (keyposXright + keypercentoffset),
          ...
          rectMode(CENTER);
          rect(
              (float) (keyposXleft + keyrectXoffset),
              (float) (keyposYleft - keyrectYoffset),
              15,
              15);
          ...
          line(
              ...
              (float) (keyposYleft - keylineYoffset),
              (float) (keyposXleft + keylineXoffset),
          ...
          text(
              strpercent,
              (float) (keyposXleft - keypercentoffset),
              (float) (keyposYleft - 2 + keydataoffset));
    ...
    stroke(175);
    line(
        (float) (keyposXright - keylineXoffset),
        (float) (keyposYright - keylineYoffset),
        (float) (keyposXright - keylineXoffset),
        (float) (keyposYright - keylineYoffset + keylinelength));
    fill(125);
    textSize(18);
    textAlign(LEFT);
    ...
    text(
        mischours + " hrs " + miscminutes + " mins ",
        (float) keyposXright,
        (float) (keyposYright - 2 + keydataoffset));
    ...
    text(
        (LocalDate.parse(TaskInfo.taskDay, DateTimeFormatter.ofPattern("yyyy-MM-dd")))
            .format(date_format),
        width / 2,
        height - 18);
  }
